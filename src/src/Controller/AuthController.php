...
class AuthController extends AbstractController
{
    ...
    public function register(Request $request)
    {
        ...
        if (empty($username) || empty($email) || empty($password) ){
            return $this->responseService->respondValidationError("Kullanıcı Adı veya E-posta ya da Parola yanlış!");
        }

        $this->userRepository->addUser($firstName, $lastName, $username, $email, $password);

        return $this->responseService->respondWithSuccess('Kullanıcı başarıyla oluşturuldu.');
    }
